...
public:
    ...
    T* GetMain();
    void PrintAddresses();      //Вывести на экран адреса указателей, записанные в каждой ячейке массива ТОЛЬКО ДЛЯ ОТЛАДКИ
...
template<typename T> void LinkStorage<T>::AddUser()
{
    ...
    if (m_count == 1) { m_storage[0][0] = new T; return; }  //Если это первый добавленный пользователь, то создается только чат с "самим собой", т.е. Заметки
    else
    {
