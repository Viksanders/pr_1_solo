...
public:
    ...
    bool isOpened();        //проверить статус флага открытости чата
    void work();            //main loop
private:
    UserStorage m_users;
    LinkStorage<ChatBox> m_links;
    User* m_YOU;              //указатель на текущего залогиненного пользователя
    ChatBox* m_CHAT;          //указатель на текущий активный чат
